...
public final class LancePageToArrowConverter
{
    ...
    private static void writeArrayElements(Block arrayBlock, FieldVector dataVector, Type elementType, int offset, int length)
    {
        ...
        if (elementType.equals(INTEGER)) {
            ...
            // Fail loudly rather than silently writing nothing while the list offsets advance,
            // which would corrupt the array. Add more element types above as needed.
            throw new TrinoException(
                    NOT_SUPPORTED,
                    format("Unsupported array element type for writing to Arrow: %s", elementType));
        }
    }
